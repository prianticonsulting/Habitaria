...
*/
		    ...
		    data[0] = {label: "Ingreso por cuota mensual", data:Math.floor(Math.random() * 100) + 1};
			data[1] = {label: "Ingreso por aportacion extraordinaria", data:Math.floor(Math.random() * 100) + 1};
            
			$.plot('#placeholder5', Ingre, {
        ...
        window.initFloatchartDemo2 = function () {
           ...
           /* for (i = 0; i < series6; i++) {
			...
			data[0] = {label: "Pago de nómina guardias", data:Math.floor(Math.random() * 100) + 1};
			data[1] = {label: "Pago de energía eléctrica", data:Math.floor(Math.random() * 100) + 1};
			data[2] = {label: "Pago de corte de cesped área común", data:Math.floor(Math.random() * 100) + 1};
			data[3] = {label: "Pago de utensilios de limpieza", data:Math.floor(Math.random() * 100) + 1};
			data[4] = {label: "Pago mtto alberca", data:Math.floor(Math.random() * 100) + 1};
			
            $.plot('#placeholder6', Egre, {
                colors: ["#ffa500", "#f87aa0", "#5bc0de", "#82b964", "#ccd600", "#f4cc13", "#8960a7", "#454b52", "#d24d33", "#f0ad4e"],
                series: {
                    pie: {
                        show: true,
        ...
        function inbox() {
            ...
            $('.mark_read, .mark_unread').click(function (e) {
                ...
                var state = $(this).is('.mark_read') ? ' Leidos' : ' No Leidos';
                var info = $checked.length + ' ' + msg + ' marcado(s) como ' + state;
                showAlert(info);
            });
